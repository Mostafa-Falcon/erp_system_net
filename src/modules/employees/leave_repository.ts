...
export class LeaveRepository {
  ...
  public static async addLeave(leave: Omit<EmployeeLeave, 'id' | 'created_at' | 'updated_at' | 'sync_status'>): Promise<void> {
  ...
  public static async updateStatus(id: string, status: EmployeeLeave['status'], approvedBy?: string): Promise<void> {
    const now = new Date().toISOString();
    await db.transaction('rw', [db.employee_leaves, db.sync_queue], async () => {
