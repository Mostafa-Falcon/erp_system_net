'use client';

import React from 'react';
import { Button } from '@/components/ui/button';
import { Badge } from '@/components/ui/badge';
import { Input } from '@/components/ui/input';
import { Checkbox } from '@/components/ui/checkbox';
import {
  Popover,
  PopoverContent,
  PopoverTrigger,
} from '@/components/ui/popover';
import {
  Table,
  TableBody,
  TableCell,
  TableHead,
  TableHeader,
  TableRow,
} from '@/components/ui/table';
import {
  Select,
  SelectContent,
  SelectItem,
  SelectTrigger,
  SelectValue,
} from '@/components/ui/select';
import {
  Calendar,
  Printer,
  FileSpreadsheet,
  FileText,
  Menu,
  SlidersHorizontal,
  Search,
  Barcode,
  ChevronLeft,
  ChevronRight,
  ChevronsLeft,
  ChevronsRight,
} from 'lucide-react';
import { formatNumber, isExpired, daysToExpiry } from '@/lib/format';
import { cn } from '@/lib/utils';
import type { ProductBatch } from '@/types';
import { TableDensity, BatchColumnsState } from './types';

interface BatchesTabProps {
  density: TableDensity;
  setDensity: (d: TableDensity) => void;
  batchColumns: BatchColumnsState;
  setBatchColumns: React.Dispatch<React.SetStateAction<BatchColumnsState>>;
  batchSearch: string;
  setBatchSearch: (q: string) => void;
  batchPageSize: number;
  setBatchPageSize: (s: number) => void;
  batchCurrentPage: number;
  setBatchCurrentPage: React.Dispatch<React.SetStateAction<number>>;
  batchTotalPages: number;
  filteredBatches: ProductBatch[];
  paginatedBatches: ProductBatch[];
  handleBatchSort: (field: 'batch_number' | 'expiry_date' | 'current_quantity') => void;
  exportBatchesToCsv: () => void;
  rowPadding: string;
}

export function BatchesTab({
  density,
  setDensity,
  batchColumns,
  setBatchColumns,
  batchSearch,
  setBatchSearch,
  batchPageSize,
  setBatchPageSize,
  batchCurrentPage,
  setBatchCurrentPage,
  batchTotalPages,
  filteredBatches,
  paginatedBatches,
  handleBatchSort,
  exportBatchesToCsv,
  rowPadding,
}: BatchesTabProps) {
  return (
    <div className="space-y-3">
      {/* Toolbar for Batches matching Screenshot 4 */}
      <div className="bg-white dark:bg-[#131b2e] rounded-2xl border border-slate-200/80 dark:border-slate-800 p-3 shadow-xs flex flex-col md:flex-row items-center justify-between gap-3">
        <div className="flex items-center gap-2 w-full md:w-auto">
          {/* Print */}
          <Button
            variant="outline"
            size="icon"
            onClick={() => window.print()}
            title="طباعة"
            className="w-9 h-9 rounded-xl text-slate-600 border-slate-200 dark:border-slate-700 cursor-pointer"
          >
            <Printer className="w-4 h-4" />
          </Button>

          {/* CSV */}
          <Button
            variant="outline"
            size="icon"
            onClick={exportBatchesToCsv}
            title="تصدير ملف CSV"
            className="w-9 h-9 rounded-xl text-emerald-600 border-emerald-200 hover:bg-emerald-50 dark:hover:bg-emerald-950/40 cursor-pointer"
          >
            <FileText className="w-4 h-4" />
          </Button>

          {/* Excel */}
          <Button
            variant="outline"
            size="icon"
            onClick={exportBatchesToCsv}
            title="تصدير إكسل Excel"
            className="w-9 h-9 rounded-xl text-emerald-700 border-emerald-300 hover:bg-emerald-50 dark:hover:bg-emerald-950/40 cursor-pointer"
          >
            <FileSpreadsheet className="w-4 h-4" />
          </Button>

          {/* Density Popover */}
          <Popover>
            <PopoverTrigger asChild>
              <Button
                variant="outline"
                size="icon"
                title="تغيير كثافة الجدول"
                className="w-9 h-9 rounded-xl text-slate-500 border-slate-200 dark:border-slate-700 cursor-pointer"
              >
                <Menu className="w-4 h-4" />
              </Button>
            </PopoverTrigger>
            <PopoverContent
              className="w-44 p-1.5 bg-white dark:bg-[#131b2e] border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xl z-50 text-right space-y-1"
              dir="rtl"
            >
              <button
                onClick={() => setDensity('compact')}
                className={cn(
                  'w-full flex items-center justify-between p-2 rounded-xl text-xs font-bold transition-colors cursor-pointer',
                  density === 'compact' ? 'bg-blue-50 text-blue-600 font-black' : 'text-slate-700'
                )}
              >
                <span>مكثف (صغير)</span>
              </button>
              <button
                onClick={() => setDensity('medium')}
                className={cn(
                  'w-full flex items-center justify-between p-2 rounded-xl text-xs font-bold transition-colors cursor-pointer',
                  density === 'medium' ? 'bg-blue-50 text-blue-600 font-black' : 'text-slate-700'
                )}
              >
                <span>متوسط (قياسي)</span>
              </button>
              <button
                onClick={() => setDensity('relaxed')}
                className={cn(
                  'w-full flex items-center justify-between p-2 rounded-xl text-xs font-bold transition-colors cursor-pointer',
                  density === 'relaxed' ? 'bg-blue-50 text-blue-600 font-black' : 'text-slate-700'
                )}
              >
                <span>مريح (واسع)</span>
              </button>
            </PopoverContent>
          </Popover>

          {/* Column Customizer for Batches matching Screenshot 3 */}
          <Popover>
            <PopoverTrigger asChild>
              <Button
                variant="outline"
                size="sm"
                className="h-9 px-3 text-xs font-black rounded-xl border-slate-200 dark:border-slate-700 gap-1.5 text-slate-600 dark:text-slate-300 shadow-2xs cursor-pointer"
              >
                <SlidersHorizontal className="w-3.5 h-3.5" />
                <span>تخصيص الأعمدة</span>
              </Button>
            </PopoverTrigger>
            <PopoverContent
              className="w-56 p-3 bg-white dark:bg-[#131b2e] border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xl z-50 text-right"
              dir="rtl"
            >
              <h4 className="text-xs font-black text-slate-900 dark:text-white mb-2 pb-1.5 border-b border-slate-100 dark:border-slate-800">
                تخصيص الأعمدة
              </h4>
              <div className="space-y-2 text-xs font-bold">
                <label className="flex items-center gap-2 cursor-pointer">
                  <Checkbox
                    checked={batchColumns.number}
                    onCheckedChange={(c) => setBatchColumns((p) => ({ ...p, number: !!c }))}
                  />
                  <span>رقم التشغيلة</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                  <Checkbox
                    checked={batchColumns.expiry}
                    onCheckedChange={(c) => setBatchColumns((p) => ({ ...p, expiry: !!c }))}
                  />
                  <span>تاريخ الانتهاء</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                  <Checkbox
                    checked={batchColumns.quantity}
                    onCheckedChange={(c) => setBatchColumns((p) => ({ ...p, quantity: !!c }))}
                  />
                  <span>الكمية المتبقية</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                  <Checkbox
                    checked={batchColumns.status}
                    onCheckedChange={(c) => setBatchColumns((p) => ({ ...p, status: !!c }))}
                  />
                  <span>الحالة</span>
                </label>
              </div>
            </PopoverContent>
          </Popover>

          {/* Page Size */}
          <div className="flex items-center gap-1.5 mr-2">
            <span className="text-[11px] font-bold text-slate-400">عرض</span>
            <Select value={String(batchPageSize)} onValueChange={(v) => setBatchPageSize(Number(v))}>
              <SelectTrigger className="h-9 w-20 rounded-xl bg-slate-50 dark:bg-slate-900 text-xs font-mono font-black">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="25">25</SelectItem>
                <SelectItem value="50">50</SelectItem>
                <SelectItem value="100">100</SelectItem>
              </SelectContent>
            </Select>
            <span className="text-[11px] font-bold text-slate-400">إدخالات</span>
          </div>
        </div>

        {/* Search Box */}
        <div className="relative w-full md:w-80">
          <div className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none">
            <Barcode className="w-4 h-4" />
          </div>
          <Input
            type="text"
            placeholder="بحث سريع في الجدول..."
            value={batchSearch}
            onChange={(e) => setBatchSearch(e.target.value)}
            className="h-10 pr-9 pl-20 bg-slate-50 dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-xs font-bold rounded-xl"
          />
          <div className="absolute left-2.5 top-1/2 -translate-y-1/2">
            <Badge className="bg-emerald-50 text-emerald-700 dark:bg-emerald-950/60 dark:text-emerald-300 border border-emerald-200 text-[10px] font-mono font-black h-5 px-1.5 flex items-center gap-1">
              <Search className="w-3 h-3 text-emerald-600" />
              <span>{filteredBatches.length}</span>
            </Badge>
          </div>
        </div>
      </div>

      {/* Table matching Screenshot 4 */}
      <div className="bg-white dark:bg-[#131b2e] rounded-2xl border border-slate-200/80 dark:border-slate-800 overflow-hidden shadow-md">
        <Table>
          <TableHeader className="bg-slate-50/90 dark:bg-slate-900/90 border-b border-slate-200/80 dark:border-slate-800">
            <TableRow>
              {batchColumns.number && (
                <TableHead
                  onClick={() => handleBatchSort('batch_number')}
                  className="text-[11px] font-black uppercase text-right cursor-pointer select-none hover:text-blue-600 transition-colors"
                >
                  <div className="flex items-center gap-1">
                    <span>رقم التشغيلة</span>
                    <span className="text-[10px] text-slate-400">⇅</span>
                  </div>
                </TableHead>
              )}
              {batchColumns.expiry && (
                <TableHead
                  onClick={() => handleBatchSort('expiry_date')}
                  className="text-[11px] font-black uppercase text-right cursor-pointer select-none hover:text-blue-600 transition-colors"
                >
                  <div className="flex items-center gap-1">
                    <span>تاريخ الانتهاء</span>
                    <span className="text-[10px] text-slate-400">⇅</span>
                  </div>
                </TableHead>
              )}
              {batchColumns.quantity && (
                <TableHead
                  onClick={() => handleBatchSort('current_quantity')}
                  className="text-[11px] font-black uppercase text-center cursor-pointer select-none hover:text-blue-600 transition-colors"
                >
                  <div className="flex items-center justify-center gap-1">
                    <span>الكمية المتبقية</span>
                    <span className="text-[10px] text-slate-400">⇅</span>
                  </div>
                </TableHead>
              )}
              {batchColumns.status && (
                <TableHead className="text-[11px] font-black uppercase text-left">الحالة ⇅</TableHead>
              )}
            </TableRow>
          </TableHeader>
          <TableBody className="text-xs font-bold divide-y divide-slate-100 dark:divide-slate-800/80">
            {paginatedBatches.length === 0 ? (
              <TableRow>
                <TableCell colSpan={4} className="py-20 text-center text-slate-400">
                  <Calendar className="w-10 h-10 opacity-20 mx-auto mb-2" />
                  <span>لا توجد أرقام تشغيلات مسجلة لهذا الصنف بعد.</span>
                </TableCell>
              </TableRow>
            ) : (
              paginatedBatches.map((b) => {
                const expired = isExpired(b.expiry_date);
                const daysLeft = b.expiry_date ? daysToExpiry(b.expiry_date) : null;
                return (
                  <TableRow key={b.id} className="hover:bg-slate-50/70 dark:hover:bg-slate-800/40">
                    {/* رقم التشغيلة */}
                    {batchColumns.number && (
                      <TableCell className={cn(rowPadding, 'font-mono font-bold text-slate-800 dark:text-slate-200 text-right')}>
                        {b.batch_number}
                      </TableCell>
                    )}

                    {/* تاريخ الانتهاء مع النص التوضيحي تحته matching Screenshot 4 */}
                    {batchColumns.expiry && (
                      <TableCell className={cn(rowPadding, 'text-right')}>
                        <div className="flex flex-col">
                          <span className="font-mono font-black text-slate-900 dark:text-white">
                            {b.expiry_date ? b.expiry_date.slice(0, 10) : '—'}
                          </span>
                          {b.expiry_date && (
                            <span className="text-[10px] text-slate-400 font-sans mt-0.5">
                              {expired
                                ? `منتهي منذ ${Math.abs(daysLeft || 0)} يوم`
                                : `ينتهي خلال ${daysLeft} يوم`}
                            </span>
                          )}
                        </div>
                      </TableCell>
                    )}

                    {/* الكمية المتبقية */}
                    {batchColumns.quantity && (
                      <TableCell className={cn(rowPadding, 'text-center font-mono font-black text-slate-900 dark:text-white')}>
                        {formatNumber(b.current_quantity)}
                      </TableCell>
                    )}

                    {/* الحالة matching Screenshot 4 */}
                    {batchColumns.status && (
                      <TableCell className={cn(rowPadding, 'text-left')}>
                        <Badge
                          className={cn(
                            'text-[10px] font-bold rounded-full px-2.5 py-0.5 border-none',
                            expired
                              ? 'bg-red-50 text-red-700 dark:bg-red-950/60'
                              : 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950/60 dark:text-emerald-300'
                          )}
                        >
                          {expired ? 'منتهي' : 'نشط'}
                        </Badge>
                      </TableCell>
                    )}
                  </TableRow>
                );
              })
            )}
          </TableBody>
        </Table>

        {/* Bottom pagination matching Screenshot 4 */}
        <div className="p-4 border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/40 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs font-bold">
          <span className="text-slate-500">
            عرض {filteredBatches.length === 0 ? 0 : (batchCurrentPage - 1) * batchPageSize + 1} إلى{' '}
            {Math.min(batchCurrentPage * batchPageSize, filteredBatches.length)} من إجمالي {filteredBatches.length} تشغيلة
          </span>

          <div className="flex items-center gap-1.5" dir="ltr">
            <Button
              variant="outline"
              size="icon"
              onClick={() => setBatchCurrentPage(1)}
              disabled={batchCurrentPage === 1}
              className="w-8 h-8 rounded-lg cursor-pointer"
            >
              <ChevronsLeft className="w-4 h-4" />
            </Button>
            <Button
              variant="outline"
              size="icon"
              onClick={() => setBatchCurrentPage((p) => Math.max(1, p - 1))}
              disabled={batchCurrentPage === 1}
              className="w-8 h-8 rounded-lg cursor-pointer"
            >
              <ChevronLeft className="w-4 h-4" />
            </Button>
            <div className="px-3 py-1 bg-white dark:bg-[#131b2e] border border-slate-200 dark:border-slate-700 rounded-lg text-xs font-mono font-black">
              {batchCurrentPage} / {batchTotalPages}
            </div>
            <Button
              variant="outline"
              size="icon"
              onClick={() => setBatchCurrentPage((p) => Math.min(batchTotalPages, p + 1))}
              disabled={batchCurrentPage >= batchTotalPages}
              className="w-8 h-8 rounded-lg cursor-pointer"
            >
              <ChevronRight className="w-4 h-4" />
            </Button>
            <Button
              variant="outline"
              size="icon"
              onClick={() => setBatchCurrentPage(batchTotalPages)}
              disabled={batchCurrentPage >= batchTotalPages}
              className="w-8 h-8 rounded-lg cursor-pointer"
            >
              <ChevronsRight className="w-4 h-4" />
            </Button>
          </div>
        </div>
      </div>
    </div>
  );
}
