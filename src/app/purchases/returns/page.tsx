...
function ReturnsContent() {
  ...
  const filtered = useMemo(() => {
    return returns.filter((ret) => {
      if (supplierFilter && ret.supplier_id !== supplierFilter) return false;
  ...
  return (
    <AppShell title="مرتجعات المشتريات" subtitle="سجل رد البضاعة للموردين — يخصم من المخزون ويعيد المبلغ أو يخصم من الذمم">
      <div className="space-y-4">
        ...
        <div className="bg-white dark:bg-[#131b2e] rounded-2xl border border-slate-200/80 dark:border-slate-800 p-4 space-y-3">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <Button onClick={() => router.push('/purchases/returns/new')} className="h-10 px-4 bg-[#558b2f] hover:bg-[#436d25] text-white rounded-lg text-xs font-bold flex items-center gap-1.5">
          ...
          {isLoading ? (
            ...
            <div className="py-12 text-center">
              ...
              <p className="text-sm font-bold text-slate-500 dark:text-slate-400">لا توجد مرتجعات مشتريات بعد.</p>
              ...
              <table className="w-full text-right">
                <thead>
                  <tr className="border-b border-slate-100 dark:border-slate-800 text-[10px] font-black text-slate-400">
                    ...
                    <th className="py-2.5 pl-3">الاسترداد</th>
                  </tr>
                </thead>
                <tbody>
                  {filtered.map((ret) => (
                    <tr key={ret.id} onClick={() => setViewId(ret.id)} className="cursor-pointer border-b border-slate-50 dark:border-slate-800/60 text-xs font-bold text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800/40">
                      ...
                      <td className="py-3">{nameOf(suppliers, ret.supplier_id)}</td>
                      ...
                      <td className="py-3 pl-3">
                        {ret.treasury_id ? (
                          <span className="text-emerald-600 dark:text-emerald-400 font-bold">استرداد نقدي</span>
                        ) : (
                          <span className="text-amber-600 dark:text-amber-400 font-bold">خصم من الذمم</span>
                        )}
                      </td>
                    </tr>
                  ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => setViewId(null)}>
          <div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl bg-white dark:bg-[#131b2e] p-6" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-4 text-xs font-bold text-slate-600 dark:text-slate-300">
              <Info label="المورد" value={nameOf(suppliers, viewReturn.supplier_id)} />
              ...
              <Info label="الاسترداد" value={viewReturn.treasury_id ? `استرداد نقدي (${nameOf(treasuries, viewReturn.treasury_id)})` : 'خصم من ذمم المورد'} />
