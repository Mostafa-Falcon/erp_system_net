          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              {editTarget.role === 'super_admin' ? (
                ...
                <select value={eRole} onChange={(e) => setERole(e.target.value as UserRole)} className={selectCls}>
                  {ROLE_OPTIONS.map((opt) => (
                    <option key={opt.value} value={opt.value}>{opt.label}</option>
                  ))}
                </select>
            ...
            <div>
              ...
              {editTarget.role === 'super_admin' ? (
                ...
                <select value={eBranchId} onChange={(e) => setEBranchId(e.target.value)} className={selectCls}>
                  {branches.map((b) => (
                    <option key={b.id} value={b.id}>{b.name}</option>
                  ))}
                </select>
